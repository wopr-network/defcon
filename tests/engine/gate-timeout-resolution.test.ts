...
describe("resolveGateTimeout", () => {
  const originalEnv = process.env.HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS;

  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS;
    } else {
      process.env.HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS = originalEnv;
    }
  });
  ...
  it("respects HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS env var", () => {
    process.env.HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS = "600000";
    expect(resolveGateTimeout(undefined, undefined)).toBe(600000);
  });

  it("falls back to 300000 when HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS is not a valid number", () => {
    process.env.HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS = "not-a-number";
    expect(resolveGateTimeout(undefined, undefined)).toBe(300000);
  });

  it("falls back to 300000 when HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS is 0", () => {
    process.env.HOLYSHIP_DEFAULT_GATE_TIMEOUT_MS = "0";
    expect(resolveGateTimeout(undefined, undefined)).toBe(300000);
  });
});
